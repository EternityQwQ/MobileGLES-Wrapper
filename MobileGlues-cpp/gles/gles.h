...
// ---------------------------------------------------------------------------
// The entry-point list, as a macro
//
// init_target_gles() resolves each driver entry point in one INIT_GLES_FUNC row
// and gles_func_t declares the slot each row writes. GLES_ALL_ENTRIES(X) is that
// same sequence of names, so X can be redefined to re-run the list for another
// purpose — gles/loader.cpp builds its unresolved-slot report from it — and a
// name can never be present in one list and missing from the other.
//
// It is generated from the INIT_GLES_FUNC list and is kept in the same ORDER as
// that list, not as gles_func_t. Order does not matter for this header, which
// only needs the names; it matters for gles/loader.cpp, whose GLES_ENTRY(X)
// expansion takes the address of each slot, and taking it in the resolution
// order is what lets a reader check the two side by side without re-sorting.
//
// gles_func_t is the third list and the only one it must agree with is the
// INIT_GLES_FUNC one: a field with no INIT_GLES_FUNC row is a slot init never
// assigns, and looking at the table above cannot tell you that. There is exactly
// one such field today, glBruh (a joke entry point declared at the end of the
// struct and never resolved, never called).
// ---------------------------------------------------------------------------
#define GLES_ALL_ENTRIES(X) \
    X(glActiveTexture) \
    X(glAttachShader) \
    ...
    X(glGetProgramBinary) \
    X(glProgramBinary) \
    X(glProgramParameteri) \
    X(glInvalidateFramebuffer) \
    X(glInvalidateSubFramebuffer) \
    X(glTexStorage2D) \
    X(glTexStorage3D) \
    X(glGetInternalformativ) \
    X(glDispatchCompute) \
    X(glDispatchComputeIndirect) \
    X(glDrawArraysIndirect) \
    X(glDrawElementsIndirect) \
    X(glFramebufferParameteri) \
    X(glGetFramebufferParameteriv) \
    X(glGetProgramInterfaceiv) \
    X(glGetProgramResourceIndex) \
    X(glGetProgramResourceName) \
    X(glGetProgramResourceiv) \
    X(glGetProgramResourceLocation) \
    ...
    X(glGetProgramPipelineInfoLog) \
    X(glBindImageTexture) \
    X(glGetBooleani_v) \
    X(glMemoryBarrier) \
    X(glMemoryBarrierByRegion) \
    X(glTexStorage2DMultisample) \
    ...
    X(glPrimitiveBoundingBox) \
    X(glGetGraphicsResetStatus) \
    X(glReadnPixels) \
    X(glGetnUniformfv) \
    X(glGetnUniformiv) \
    X(glGetnUniformuiv) \
    ...
    X(glTexBuffer) \
    X(glTexBufferRange) \
    X(glTexStorage3DMultisample) \
    X(glMapBufferRange) \
    X(glBufferStorageEXT) \
    X(glGetQueryObjectivEXT) \
    X(glGetQueryObjecti64vEXT) \
    X(glBindFragDataLocationEXT) \
    X(glMapBufferOES) \
    X(glMultiDrawArraysIndirectEXT) \
    X(glMultiDrawElementsIndirectEXT) \
    X(glMultiDrawArraysIndirect) \
    X(glMultiDrawElementsIndirect) \
    X(glMultiDrawElementsBaseVertexEXT)
